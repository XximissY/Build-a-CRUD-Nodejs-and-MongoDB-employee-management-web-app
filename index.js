...
async function findDataWithFilterNaN(collectionName, option) {
  try {
    ...
    if (option.filter) {
      const filters = Array.isArray(option.filter)
        ? option.filter
        : [option.filter];
      const filterQuery = {
        $and: [],
      ...
      filters.forEach((filter) => {
        ...
        switch (command) {
          ...
          case "ni":
            ...
            break;
          case "is":
            filterQuery.$and.push({ [column]: null });
            break;
          case "no":
            filterQuery.$and.push({
              [column]: {
    ...
    let recordsPerPage = option.page
      ? parseInt(option.page.split(",")[1])
      : 100;
...
app.get(`/apiman/v1/apollonia/:path`, async (req, res) => {
  ...
  findDataWithFilterNaN(pathParam, query)
    .then((result) => {
...
app.post(`/apiman/v1/apollonia/:path`, async (req, res) => {
  ...
  insertUpdateData(pathParam, query)
    .then((result) => {
...
app.delete(`/apiman/v1/apollonia/:path`, async (req, res) => {
  ...
  deleteDocument(pathParam, query)
    .then((result) => {
      res.status(result.statusCode);
      res.send(result.body);
    })
...
app.put(`/apiman/v1/apollonia/:path`, async (req, res) => {
  ...
  req.body._id = _id;
  updateDocument(pathParam, _id, req.body)
    .then((result) => {
      result.body.req = req.body;
      res.status(result.statusCode);
      res.send(result.body);
